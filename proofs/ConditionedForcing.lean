...
theorem conditionedClass_card_le
  ...
  · have hsub : conditionedClass N q0 q ⊆
      ...
      exact ⟨hx.1, hx.2.1⟩
    have hraw := card_filter_mod_le N q.1 q.2 hqdvd
...
/-- The core avoiding one selected prime class has the usual CRT lower bound. -/
theorem conditionedCore_card_ge
    {N : ℕ} (q0 : ℕ × ℕ)
    (hq0dvd : q0.1 ∣ N) (hq0one : 1 < q0.1) :
    (N / q0.1) * (q0.1 - 1) ≤ (conditionedCore N q0).card := by
  classical
  have h := uncovered_card_ge (N := N) ({q0} : Finset (ℕ × ℕ))
    (by intro q hq; simpa using hq0dvd)
    (by intro q hq; simpa using hq0one)
    (by intro p hp q hq hpq; simp at hp hq; subst p; subst q; contradiction)
  simpa [conditionedCore] using h
...
theorem forced_modulus_after_one_prime
  ...
  have hcore : (N / p) * (p - 1) ≤ (conditionedCore N q0).card := by
    subst p
    exact conditionedCore_card_ge q0 hpdvd hpprime.one_lt
  ...
  have hcap : ∀ q ∈ S.erase q0,
    ...
    subst p
    exact conditionedClass_card_le q0 q hpprime hpdvd hq0res (hdvd q hqS)
  ...
  have hinjErase : Set.InjOn Prod.fst ((S.erase q0 : Finset (ℕ × ℕ)) : Set (ℕ × ℕ)) :=
    fun a ha b hb hab => hinj (Finset.mem_of_mem_erase ha) (Finset.mem_of_mem_erase hb) hab
  ...
  have hsub : (S.erase q0).image Prod.fst ⊆ (Allowed.erase p).erase d0 := by
    ...
    have hnep : q.1 ≠ p := by
      intro heq
      have hfst : q.1 = q0.1 := by simpa [hq0mod] using heq
      exact hqne0 (hinj hqS hq0S hfst)
    exact Finset.mem_erase.mpr ⟨hno q hqS, Finset.mem_erase.mpr ⟨hnep, hallowed q hqS⟩⟩
